...
impl LlamaCppImageCaptionv3 {
    ...
    fn execute<'py>(
        ...
        let (pipeline_config, requests, llama_cpp_model_params) = self
            .options_parser(
                model,
                options,
                system_prompt,
...
impl LlamaCppImageCaptionv3 {
    ...
    fn options_parser<'py>(
        ...
        session_id: String,
        kwargs: Option<Bound<'py, PyDict>>,
    ) -> Result<(PipelineConfig, Vec<Request>, LlamaCppModelParams), Error> {
        let kwargs =
        ...
        Ok((pipeline_config, requests, llama_cpp_model_params))
    }
