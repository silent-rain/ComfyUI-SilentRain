...
class FluxBlockSwapPatchNode:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "double_blocks": (
                    "INT",
                    {"min": 0, "max": 19, "default": 7, "tooltip": "double blocks 19"},
                ),
                "single_blocks": (
                    "INT",
                    {"min": 0, "max": 38, "default": 7, "tooltip": "single blocks 38"},
                ),
            }
        }
    ...
    def apply_patch(self, model, double_blocks=6, single_blocks=6):
        # 应用block swap patch
        patched_model = FluxBlockSwapPatch(
            model,
