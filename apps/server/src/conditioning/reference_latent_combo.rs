...
impl ReferenceLatentCombo {
    ...
    fn return_types() -> (&'static str, &'static str, &'static str) {
        (NODE_CONDITIONING, NODE_CONDITIONING, NODE_LATENT)
    }
    ...
    #[pyo3(name = "execute", signature = (positive, negative, vae, main_image, main_long_edge, ref_long_index, ref_long_edge, ref_image1=None, ref_image2=None, ref_image3=None, ref_image4=None, ref_image5=None))]
...
impl ReferenceLatentCombo {
    ...
    fn run<'py>(
        ...
        // 1. 处理 main_image：缩放 → VAE 编码
        let main_resized = Self::resize_image(py, &main_image, main_long_edge)?;
        let main_latent = Self::vae_encode(py, &vae, &main_resized)?;

        // 2. 将 main latent 应用到 positive 和 negative
