...
const StringDynList2 = (): ComfyExtension => {
    return {
        name: NODE_NAME,
        init: async (_app) => {
            // Node initialization
        ...
        beforeRegisterNodeDef: async (nodeType, nodeData, _app) => {
            ...
            nodeType.prototype.onConnectionsChange = function (type, _index, isConnected, link_info) {
                ...
                if (isConnected) {
                    const linkCount = this.inputs.filter((slot) => slot.name !== 'delimiter').filter((slot) => !slot.link).length;
                    ...
                    // 移除所有的空闲slot
                    const inputTotal = this.inputs.length;
                    let linkCount = this.inputs.filter((slot) => slot.name !== 'delimiter').filter((slot) => !slot.link).length;
                    console.log(`inputTotal: ${inputTotal}, linkCount: ${linkCount}`);
