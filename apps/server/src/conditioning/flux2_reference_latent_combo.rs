//! Flux2 Reference Latent Combo
//!
//! 组合节点：专用于 Flux2 reference latent。
//! 将多张参考图像通过 ResizeImageMaskByLongerEdge + VAEEncode/VAEEncodeForInpaint/EmptyFlux2LatentImage
//! 进行链式拼接，生成带有参考 latent 的 conditioning 输出。
//!
//! 节点流程：
//!   main_image (+ main_mask) → ResizeImageMaskByLongerEdge(main_long_edge)
//!   → GetImageSize → 若有 main_mask: VAEEncodeForInpaint → latent
//!                    否则: EmptyFlux2LatentImage(width, height) → latent
//!   → ReferenceLatent(positive/negative)
//!   ref_imageN → ResizeImageMaskByLongerEdge(ref_long_edge 或 main_long_edge)
//!   → VAEEncode → latent → ReferenceLatent(positive/negative)
...
/// Flux2 Reference Latent Combo 组合节点
#[pyclass(subclass)]
pub struct Flux2ReferenceLatentCombo {}

impl PromptServer for Flux2ReferenceLatentCombo {}

#[pymethods]
impl Flux2ReferenceLatentCombo {
    #[new]
    fn new() -> Self {
        Self {}
    }
    ...
    fn description() -> &'static str {
        "Flux2 combo node: chains multiple reference images through ResizeImageMaskByLongerEdge + VAEEncode + ReferenceLatent. \
        Main image uses VAEEncodeForInpaint (if main_mask provided) or EmptyFlux2LatentImage. \
    ...
    fn execute<'py>(
        ...
        match result {
            Ok(v) => Ok(v),
            Err(e) => {
                error!("Flux2ReferenceLatentCombo error: {e}");
                if let Err(send_err) =
                    self.send_error(py, "Flux2ReferenceLatentCombo".to_string(), e.to_string())
                {
...
impl Flux2ReferenceLatentCombo {
    ...
    /// 通过 Python 调用 VAEEncodeForInpaint 进行 inpaint 编码
    /// 返回 latent dict {"samples": tensor, "noise_mask": tensor}
    fn vae_encode_for_inpaint<'py>(
        py: Python<'py>,
        vae: &Bound<'py, PyAny>,
        ...
        let node_cls = module.getattr("VAEEncodeForInpaint")?;
        let node_inst = node_cls.call0()?;
        let result = node_inst.call_method1("encode", (vae, pixels, mask))?;
        // encode 返回 (latent_dict,)，取第一个元素
        let latent = result.get_item(0)?;
        Ok(latent)
    }

    /// 通过 Python 调用 EmptyFlux2LatentImage 生成空 latent
    /// 返回 latent dict {"samples": tensor}
    fn empty_flux2_latent<'py>(
        py: Python<'py>,
        width: i64,
        height: i64,
    ) -> Result<Bound<'py, PyAny>, Error> {
        let module = py.import("comfy_extras.nodes_flux")?;
        let node_cls = module.getattr("EmptyFlux2LatentImage")?;
        let result = node_cls.call_method1("execute", (width, height))?;
        // execute 返回 NodeOutput(latent_dict)，取第一个元素
        let latent = result.get_item(0)?;
        Ok(latent)
    }
    ...
    > {
        ...
        // 4. 将 main latent 应用到 positive 和 negative
        ...
        for (i, ref_image_opt) in ref_images.into_iter().enumerate() {
            ...
            let (ref_resized, _) = Self::resize_image_mask(py, &ref_image, edge, None)?;
