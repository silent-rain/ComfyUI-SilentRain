...
// JSON data structure for Kontext presets
lazy_static::lazy_static! {
    pub static ref KONTEXT_PRESETS_DATA: Value = json!({
...
impl LoadKontextPresetsAssistant {
    ...
    fn input_types(_cls: &Bound<'_, PyType>) -> PyResult<Py<PyDict>> {
        Python::with_gil(|py| {
            ...
            dict.set_item("required", {
                ...
                let mut preset_options = Vec::new();
                for preset in KONTEXT_PRESETS_DATA["presets"]
                    .as_array()
                    .unwrap_or(&Vec::new())
                {
                    if let Some(name) = preset["name"].as_str() {
                        preset_options.push(name.to_string());
                    }
                }
                required.set_item(
                    "preset",
    ...
    #[pyo3(name = "execute")]
    fn execute<'py>(&mut self, py: Python<'py>, preset: &str) -> PyResult<(String,)> {
...
impl LoadKontextPresetsAssistant {
    /// 获取预设
    fn get_preset(&self, preset_name: &str) -> Result<(String,), Error> {
        let preset_assistant: PresetAssistant =
            serde_json::from_value(KONTEXT_PRESETS_DATA.clone())?;
